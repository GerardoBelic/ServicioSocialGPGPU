/**
TODO: In one line I made input.iterm equal to 0 to eliminate the random changes to xoc.v (velocities),
      and related to that I made the getUniformRandom() function return non-random numbers to control the debug of the
...
/// TODO: delete this constants because the program doesn't rely on fixed size arrays
//#define NATMAX 5000
...
struct Distancies
{
    ...
    {
        ///TODO: For now the values are hardcoded
...
/// TODO: DELETE the lines that make the function not random
...
/// Gets an index in the range of number of atoms [0 - natom-1]
auto getAtomInRange(int natom) -> int
{

    static std::random_device rd;
    static std::mt19937 gen(rd());
    std::uniform_int_distribution<int> num_in_range(0, natom - 1);

    return num_in_range(gen);

}

/// TODO: verify all structs (Xoc, Pous, ...., Parmsolv, Other) and check if its sizes depend on number of atoms or nmber of residues (or number of pairs in case of topology)
/// TODO: also change the type of some arrays to be more explicit, like the ints used as bools
...
struct Xoc
{
    ...
    std::vector<double> xm; ///Value. Size of natom. Hold an atom energy
...
struct Cov
{
    Cov(int natom) : icov(natom, std::vector<int>(natom)), rhc(natom) { }
...
auto potencial(int natom, Xoc &xoc, Pous &pous, Intr &intr, Cov &cov, Pdb &pdb, Fisic &fisic, Param &param, Parmsolv &parmsolv) -> void
{
    ...
    double rbox2 = 0.5 * xoc.rbox;
    int nres = pdb.ind2[natom - 1] + OFFSET;    /// Unused
    ...
    for (int i = 0; i < natom; ++i)
    {
        ...
        {{
            {xoc.r[i][1 - OFFSET] + parmsolv.dwat, xoc.r[i][2 - OFFSET]                , xoc.r[i][3 - OFFSET]                },
            {xoc.r[i][1 - OFFSET] - parmsolv.dwat, xoc.r[i][2 - OFFSET]                , xoc.r[i][3 - OFFSET]                },
            {xoc.r[i][1 - OFFSET]                , xoc.r[i][2 - OFFSET] + parmsolv.dwat, xoc.r[i][3 - OFFSET]                },
            {xoc.r[i][1 - OFFSET]                , xoc.r[i][2 - OFFSET] - parmsolv.dwat, xoc.r[i][3 - OFFSET]                },
            {xoc.r[i][1 - OFFSET]                , xoc.r[i][2 - OFFSET]                , xoc.r[i][3 - OFFSET] + parmsolv.dwat},
            {xoc.r[i][1 - OFFSET]                , xoc.r[i][2 - OFFSET]                , xoc.r[i][3 - OFFSET] - parmsolv.dwat}
        }};

        std::array<std::array<double, 3>, 8> rv =
        {{
            {xoc.r[i][1 - OFFSET] + dwatd,   xoc.r[i][2 - OFFSET] + dwatd,    xoc.r[i][3 - OFFSET] + dwatd},
            {xoc.r[i][1 - OFFSET] + dwatd,   xoc.r[i][2 - OFFSET] + dwatd,    xoc.r[i][3 - OFFSET] - dwatd},
            {xoc.r[i][1 - OFFSET] + dwatd,   xoc.r[i][2 - OFFSET] - dwatd,    xoc.r[i][3 - OFFSET] + dwatd},
            {xoc.r[i][1 - OFFSET] + dwatd,   xoc.r[i][2 - OFFSET] - dwatd,    xoc.r[i][3 - OFFSET] - dwatd},
            {xoc.r[i][1 - OFFSET] - dwatd,   xoc.r[i][2 - OFFSET] + dwatd,    xoc.r[i][3 - OFFSET] + dwatd},
        ...
        for (int r_ind = 0; r_ind < r.size(); ++r_ind)
        {
            for (int j = 0; j < natom; ++j)
            {
                ...
                double rij1 = r[r_ind][1 - OFFSET] - xoc.r[j][1 - OFFSET];
                ...
                double rij2 = r[r_ind][2 - OFFSET] - xoc.r[j][2 - OFFSET];
                ...
                double rij3 = r[r_ind][3 - OFFSET] - xoc.r[j][3 - OFFSET];
        ...
        for (int rv_ind = 0; rv_ind < rv.size(); ++rv_ind)
        {
            for (int j = 0; j < natom; ++j)
            {
                ...
                double rij1 = rv[rv_ind][1 - OFFSET] - xoc.r[j][1 - OFFSET];
                ...
                double rij2 = rv[rv_ind][2 - OFFSET] - xoc.r[j][2 - OFFSET];
                ...
                double rij3 = rv[rv_ind][3 - OFFSET] - xoc.r[j][3 - OFFSET];
    ...
    {
        //int ii = pdb.ind2[i];   /// Unused

        for (int j = i + 1; j < natom; ++j)
        {
            //int jj = pdb.ind2[j];   /// Unused
            ...
            pous.rstep[i][j][1 - OFFSET] = 0.9 * rvdwij;
            pous.rstep[i][j][2 - OFFSET] = 1.1 * rvdwij;

            if (eij < 0.0)
            {
                pous.estep[i][j][1 - OFFSET] = 3.0 * eij;
                pous.estep[i][j][2 - OFFSET] = -eij;
            }
            else
            {
                pous.estep[i][j][1 - OFFSET] = -eij;
                pous.estep[i][j][2 - OFFSET] = -eij;
            }
        }
    }
...
auto enchufa(int natom, double dcut, Xoc &xoc, Intr &intr, Cov &cov /** Unused */, Atpres &atpres, Pdb &pdb, Npt &npt) -> void
{
...
auto creapouhb(int n1, int n2, double rmin, double r0 /** Unused */, double r1 /** Unused */, double rmax, double ehb, Pous &pous, Intr &intr) -> void
{
    ...
    pous.rstep[n1][n2][1 - OFFSET] = rmin;
    pous.rstep[n1][n2][2 - OFFSET] = rmax;

    pous.estep[n1][n2][1 - OFFSET] = -1.5 * ehb;
    pous.estep[n1][n2][2 - OFFSET] = 1.5 * ehb;
...
auto chgmom(int mem1, int mem2, double rij1, double rij2, double rij3, Xoc &xoc) -> void
{
    ...
    vdmod = vdmod + (xoc.v[mem2][1 - OFFSET] - xoc.v[mem1][1 - OFFSET]) * rij1;
    vdmod = vdmod + (xoc.v[mem2][2 - OFFSET] - xoc.v[mem1][2 - OFFSET]) * rij2;
    vdmod = vdmod + (xoc.v[mem2][3 - OFFSET] - xoc.v[mem1][3 - OFFSET]) * rij3;
    ...
    xoc.v[mem1][1 - OFFSET] = xoc.v[mem1][1 - OFFSET] + dp / xoc.xm[mem1] * rij1;
    xoc.v[mem2][1 - OFFSET] = xoc.v[mem2][1 - OFFSET] - dp / xoc.xm[mem2] * rij1;

    xoc.v[mem1][2 - OFFSET] = xoc.v[mem1][2 - OFFSET] + dp / xoc.xm[mem1] * rij2;
    xoc.v[mem2][2 - OFFSET] = xoc.v[mem2][2 - OFFSET] - dp / xoc.xm[mem2] * rij2;

    xoc.v[mem1][3 - OFFSET] = xoc.v[mem1][3 - OFFSET] + dp / xoc.xm[mem1] * rij3;
    xoc.v[mem2][3 - OFFSET] = xoc.v[mem2][3 - OFFSET] - dp / xoc.xm[mem2] * rij3;
...
auto dmdshake(int natom, int nbound, int mem1 /** Unused */, int mem2 /** Unused */, Xoc &xoc, Cov &cov, Shake &shake) -> void
{
    ...
    for (int i = 0; i < natom - 1; ++i)
    {
        for (int l = 0; l < shake.ishk[i]; ++l)
        {
            ...
            double rij1 = dbox(j, i, 1 - OFFSET, xoc);
            double rij2 = dbox(j, i, 2 - OFFSET, xoc);
            double rij3 = dbox(j, i, 3 - OFFSET, xoc);

            double rij = std::sqrt(rij1 * rij1 + rij2 * rij2 + rij3 * rij3);

            double vij1 = xoc.v[i][1 - OFFSET] - xoc.v[j][1 - OFFSET];
            double vij2 = xoc.v[i][2 - OFFSET] - xoc.v[j][2 - OFFSET];
            double vij3 = xoc.v[i][3 - OFFSET] - xoc.v[j][3 - OFFSET];
    ...
    {
        int i = cov.ibound[k][1 - OFFSET];
        int j = cov.ibound[k][2 - OFFSET];
...
/// TODO: return the value instead of changing the reference
...
/**
    Function that takes the path to the molecule info (the .pdb file that list the atoms) and returns the atom info
*/
auto get_molecule_info(const std::string &molecule_file_path, const Input &input) -> std::tuple<int, Pdb, Xoc, Other>
{
    ...
    std::ifstream file(molecule_file_path, std::ios::in);
...
/**
    Function that gets the index of the elements of interest (like H, O, C, ...).
    Additionaly we get the number of "molecules" and
    the "absolute" residue index (in the .pdb file the residues are relative to each molecule).
*/
auto set_residue_indexes(int natom, Pdb &pdb, const Other &other) -> std::tuple<int, Atpres>
{

    int kk = 0; /// Temp variable for storing the sum of previous molecule's residue sequence numbers (offsets)
...
auto get_atom_types(const std::string &atom_types_file_path, int natom, Pdb &pdb, Other &other) -> void
{
    ...
    */
    ...
    std::ifstream file16(atom_types_file_path, std::ios_base::in);
    ...
    for (int i = 0; i < natom; ++i)
    {
        ...
        if (pdb.atom[i] == "N" || pdb.atom[i] == "H" || pdb.atom[i] == "C" || pdb.atom[i] == "O" || pdb.atom[i] == "OXT")
        {
            ...
            /*if      (pdb.atom[i] == "N") other.atp[i][1 - OFFSET] = "nh";
            else if (pdb.atom[i] == "H") other.atp[i][1 - OFFSET] = "h";
            else if (pdb.atom[i] == "C") other.atp[i][1 - OFFSET] = "co";
            else if (pdb.atom[i] == "O" || pdb.atom[i] == "OXT") other.atp[i][1 - OFFSET] = "oc";*/

            if      (pdb.atom[i] == "N") other.atp[i].push_back("nh");
            else if (pdb.atom[i] == "H") other.atp[i].push_back("h");
            else if (pdb.atom[i] == "C") other.atp[i].push_back("co");
            else if (pdb.atom[i] == "O" || pdb.atom[i] == "OXT") other.atp[i].push_back("oc");
        ...
        while (true)
        {
            ...
            if (pdb.atom[i] == name && pdb.res[i] == residue_name)
            {
                ...
                other.atp[i].push_back(atp_name);
                ++pdb.nat[i];
            }
        }
...
auto get_atom_parameters(const std::string &atom_parameters_file_path, int natom, const Pdb &pdb, Other &other) -> void
{

    /// TODO: Same as assign_atom_types(), we are reading the file multiple times
    std::ifstream file17(atom_parameters_file_path, std::ios_base::in);
        ...
        for (int j = 0; j < pdb.nat[i]; ++j)
        {
            while (true)
            {
                ...
                if (other.atp[i][j] == atp_name)
                {
                    ...
                    other.rhca[i][j] = 0.8 * xrvdw;
                    other.xma[i][j] = xmassa;*/

                    other.qa[i].push_back(xq);
                    other.gfreea[i].push_back(xfree);
                    other.va[i].push_back(xvol);
                    other.evdwa[i].push_back(xevdw);
                    other.rvdwa[i].push_back(xrvdw);
                    other.rhca[i].push_back(0.8 * xrvdw);
                    other.xma[i].push_back(xmassa);
...
auto set_atom_parameters(int natom, Xoc &xoc, const Pdb &pdb, const Other &other, const Input &input) -> std::tuple<double, Cov, Fisic>
{
    ...
    for (int i = 0; i < natom; ++i)
    {
        ...
        if (pdb.nat[i] == 1)
        {
            xoc.xm[i] = other.xma[i][1 - OFFSET];

            fisic.qq[i] = other.qa[i][1 - OFFSET];
            fisic.vol[i] = other.va[i][1 - OFFSET];
            fisic.gfree[i] = other.gfreea[i][1 - OFFSET];
            fisic.evdw[i] = other.evdwa[i][1 - OFFSET];
            fisic.rvdw[i] = other.rvdwa[i][1 - OFFSET];
...
auto get_topology_matrix(const std::string &topology_file_path, Cov &cov) -> int
{

    std::ifstream file7(topology_file_path, std::ios_base::in);
    ...
    while (file7)
    {
        ...
        if (file7.eof() || topology_record == "")  /// TODO: add to other input files if we are reading an empty line
            break;
        ...
        cov.icov[i][j] = 1;
        /*cov.ibound[npair][1 - OFFSET] = i ;
        cov.ibound[npair][2 - OFFSET] = j ;
        cov.rbound[npair] = rij;*/
        std::array<int, 2> ibound_ij = {i, j};
        cov.ibound.push_back(ibound_ij);
        cov.rbound.push_back(rij);
...
auto stablish_hydrogen_bonds(int nres, const Xoc &xoc, const Pdb &pdb, Atpres &atpres, const Distancies &distancies) -> int
{
    ...
    for (int i = 0; i < nres - 4; ++i)
    {
        ...
        for (int j = i + 4; j < nres; ++j)
        {
            ...
            double rij1 = dbox(n2, n1, 1 - OFFSET, xoc);
            double rij2 = dbox(n2, n1, 2 - OFFSET, xoc);
            double rij3 = dbox(n2, n1, 3 - OFFSET, xoc);
            ...
            rij1 = dbox(n2, n1, 1 - OFFSET, xoc);
            rij2 = dbox(n2, n1, 2 - OFFSET, xoc);
            rij3 = dbox(n2, n1, 3 - OFFSET, xoc);
...
auto assign_initial_overlaps(int natom, double rshake2, double rpot2, const Xoc &xoc, const Intr &intr, const Cov &cov, Shake &shake, Npt &npt) -> void
{
    ...
    for (int i = 0; i < natom - 1; ++i)
    {
        ...
        for (int j = i + 1; j < natom; ++j)
        {
            if (cov.icov[i][j] == 0)
            {
                double rij1 = dbox(i, j, 1 - OFFSET, xoc);
                double rij2 = dbox(i, j, 2 - OFFSET, xoc);
                double rij3 = dbox(i, j, 3 - OFFSET, xoc);
...
auto assign_initial_interaction_regions(int natom, const Xoc &xoc, const Intr &intr, const Pous &pous, const Cov &cov, Other &other) -> void
{
...
auto obtain_initial_conformation_potential_energy(int natom, const Xoc &xoc, const Intr &intr, const Pous &pous, const Pdb &pdb) -> std::tuple<double, double, double, double>
{
...
auto adjust_initial_velocities_and_energies(int natom, double xmassa, double epot0, Xoc &xoc, Other &other, const Input &input, const Constants &constants) -> std::tuple<double, double, double, double>
{
    ...
    for (int j = 0; j < 3; ++j)
    {
        for (int i = 0; i < natom; ++i)
        {
            ...
            ekin = ekin + 0.5 * xoc.xm[i] * std::pow(xoc.v[i][j] * constants.a, 2.0);
        }
    }
    ...
    for (int j = 0; j < 3; ++j)
    {
        for (int i = 0; i < natom; ++i)
        {
            ...
            ekin0 = ekin0 + 0.5 * xoc.xm[i] * std::pow(xoc.v[i][j] * constants.a, 2.0);
        }
    }
...
int main()
{
    ...
    auto [nres, atpres] = set_residue_indexes(natom, pdb, other);
    ...
    auto [xmassa, cov, fisic] = set_atom_parameters(natom, xoc, pdb, other, input);

    /// llegeix la matriu de topologia (topologia.dat)
    int npair = get_topology_matrix(input.file7, cov);
    ...
    */
    ...
    int nhb = stablish_hydrogen_bonds(nres, xoc, pdb, atpres, distancies);
    ...
    auto [epot0, epotmol0, epothb0, epothbmol0] = obtain_initial_conformation_potential_energy(natom, xoc, intr, pous, pdb);

    /// assigna velocitats aleatories i ajusta l'energia cinetica a la temperatura requerida
    auto [ekin, sto, ekin0, etot0] = adjust_initial_velocities_and_energies(natom, xmassa, epot0, xoc, other, input, constants);

    /// escriu les coordenades en el SRCM
    int ibloc = 0;

    std::ofstream file_input("input.pdb", std::ios_base::out);
	...
	for (int i = 0; i < natom; ++i)
    {
        ...
        file_input << boost::format("%4s  %5d  %-3s %3s %c %3d    %8.3f%8.3f%8.3f%8.3f %2d") % "ATOM" % (i + OFFSET) % pdb.atom[i] % pdb.res[i] % other.cad[i] % (other.ind1[i] + OFFSET) %
                                    xoc.r[i][1 - OFFSET] % xoc.r[i][2 - OFFSET] % xoc.r[i][3 - OFFSET] % parmsolv.fcont[i] % parmsolv.icont[i] << std::endl;

        file20 << boost::format("%4s  %5d  %-3s %3s %c %3d    %8.3f%8.3f%8.3f") % "ATOM" % (i + OFFSET) % pdb.atom[i] % pdb.res[i] % other.cad[i] % (other.ind1[i] + OFFSET) %
                                    xoc.r[i][1 - OFFSET] % xoc.r[i][2 - OFFSET] % xoc.r[i][3 - OFFSET] << std::endl;

        if (pdb.atom[i] == "CA")
        {
            file21 << boost::format("%4s  %5d  %-3s %3s %c %3d    %8.3f%8.3f%8.3f") % "ATOM" % (i + OFFSET) % pdb.atom[i] % pdb.res[i] % other.cad[i] % (other.ind1[i] + OFFSET) %
                                    xoc.r[i][1 - OFFSET] % xoc.r[i][2 - OFFSET] % xoc.r[i][3 - OFFSET] << std::endl;
        }
    }
    ...
    std::ofstream file8(input.file8, std::ios_base::out); ///TODO: rename to file8 if it doesn't exist
	//write(8,INPUT)    ///TODO: write input to file8
	//write(8,DISTANCIES)   ///TODO: write input to file8
	file8 << "natom=" << natom << " nres=" << nres << " tsnap=" << input.tsnap << " nbloc=" << input.nbloc << std::endl;
	file8 << "epot=" << epot0 << " ekin=" << ekin0 << " etot=" << etot0 << " epothb=" << epothb0 << " nhb=" << nhb << std::endl;
    ...
    for (int ibloc = 1; ibloc <= input.nbloc; ++ibloc)
    {
        ...
        while (tacum < input.tsnap)
        {
            ...
            for (int i = 0; i < natom - 1; ++i)
            {
                for (int j = i + 1; j < natom; ++j)
                {
                    ...
                    {
                        double rmin2 = pous.rstep[i][j][1 - OFFSET] * pous.rstep[i][j][1 - OFFSET];
                        double rmax2 = pous.rstep[i][j][2 - OFFSET] * pous.rstep[i][j][2 - OFFSET];
            ...
            while (tacene < input.tene)
            {
                ...
                for (int i = 0; i < natom - 1; ++i)
                {
                    for (int j = i + 1; j < natom; ++j)
                    {
                        ...
                        //double vmod2 = vij1 * vij1 + vij2 * vij2 + vij3 * vij3; /** Unused */
                        ...
                        {
                            ++icont;
                            double dpot = -pous.estep[i][j][k0 - 1] * constants.facte;
                        ...
                        {
                            ++icont;
                            double dpot = pous.estep[i][j][k0] * constants.facte;
                ...
                input.iterm = 0;    /// TODO: DELETE THIS to return the randomness to the program
                ...
                if (input.iterm == 1)
                {
                    ...
                    /// TODO: delete the code above and select a random index with the function below
                    //int i = getAtomInRange(natom);
            ...
            double ekin2 = ekin / constants.facte;
        ...
        file8 << "Temps " << temps << " hbonds " << nhb << " epot " << epot_i << " " << etot_i << std::endl;
    }
    ...
    for (int i = 0; i < natom; ++i)
    {
        file19 << boost::format("%4s  %5d  %-3s %3s %c %3d    %8.3f%8.3f%8.3f") % "ATOM" % (i + OFFSET) % pdb.atom[i] % pdb.res[i] % other.cad[i] % (other.ind1[i] + OFFSET) %
                                    xoc.r[i][1 - OFFSET] % xoc.r[i][2 - OFFSET] % xoc.r[i][3 - OFFSET] << std::endl;
    }
