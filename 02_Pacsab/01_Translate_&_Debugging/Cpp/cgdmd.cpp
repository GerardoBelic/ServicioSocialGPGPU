...
/// TODO: DELETE the lines that make the function
...
if (mem1==14||mem2==14)std::cout << mem1 << " " << mem2 << " => " << xoc.v[mem1][0]*1E-10 << " " << xoc.v[mem2][0]*1E-10 << std::endl;*/
    ...
    vdmod = vdmod + (xoc.v[mem2][0] - xoc.v[mem1][0]) * rij1;
    vdmod = vdmod + (xoc.v[mem2][1] - xoc.v[mem1][1]) * rij2;
    vdmod = vdmod + (xoc.v[mem2][2] - xoc.v[mem1][2]) * rij3;
    ...
    xoc.v[mem1][0] = xoc.v[mem1][0] + dp / xoc.xm[mem1] * rij1;
    xoc.v[mem2][0] = xoc.v[mem2][0] - dp / xoc.xm[mem2] * rij1;

    xoc.v[mem1][1] = xoc.v[mem1][1] + dp / xoc.xm[mem1] * rij2;
    xoc.v[mem2][1] = xoc.v[mem2][1] - dp / xoc.xm[mem2] * rij2;

    xoc.v[mem1][2] = xoc.v[mem1][2] + dp / xoc.xm[mem1] * rij3;
    xoc.v[mem2][2] = xoc.v[mem2][2] - dp / xoc.xm[mem2] * rij3;
...
//if ((mem1==1051)&&(mem2==1076))std::cout << boost::format("%45.40f %45.40f")%(dp*1e-10)%(std::sqrt(sto)*1e-10) << std::endl;
    if (sto > 0.0)
    {
...
//if ((mem1==1051)&&(mem2==1076))std::cout << boost::format("%45.40f")%(dp*1e-10) << std::endl;
//if ((mem1==1051)&&(mem2==1076))std::cout << boost::format("%45.40f %45.40f %45.40f  %45.40f %45.40f")%rmod2%(vdmod*1e-10)%(sto*1e-20)%(xoc.xm[mem1])%(xoc.xm[mem2]) << std::endl;
    xoc.v[mem1][0] = xoc.v[mem1][0] + (dp / xoc.xm[mem1]) * rij1;
    xoc.v[mem2][0] = xoc.v[mem2][0] - (dp / xoc.xm[mem2]) * rij1;
    ...
    */
    ...
    for (int j = 0; j < 3; ++j)
    {
        ...
        for (int i = 0; i < natom; ++i)
        {
            ...
            other.vcm[j] = other.vcm[j] + xoc.xm[i] * xoc.v[i][j];//std::cout << xoc.v[i][j] << " " << other.vcm[j] << std::endl;
        }
...
//imprimir_v(natom,1,xoc);
    ...
    for (int j = 0; j < 3; ++j)
    {
        for (int i = 0; i < natom; ++i)
        {
            ...
            ekin = ekin + 0.5 * xoc.xm[i] * std::pow(xoc.v[i][j] * input.a, 2.0);//std::cout << boost::format("%20.16f %20.16f %20.16f %20.16f")%((0.5 * xoc.xm[i] * std::pow(xoc.v[i][j] * input.a, 2.0))*1.E24) % ((xoc.v[i][j] * input.a)*1.E11) % ((std::pow(xoc.v[i][j] * input.a, 2.0))*1.E21) % (xoc.v[i][j]) << std::endl;
        }
...
//std::cout << boost::format("%40.35f %40.35f %40.35f %40.35f")%1.e-11f %1.e-11%(double(1.e-11f))%(float(1.e-11));std::terminate();
    ...
    for (int j = 0; j < 3; ++j)
    {
        for (int i = 0; i < natom; ++i)
        {
            ...
            ekin0 = ekin0 + 0.5 * xoc.xm[i] * std::pow(xoc.v[i][j] * input.a, 2.0);//std::cout << xoc.v[i][j] << " " << ekin0 << std::endl;
        }
    }
...
/*std::cout << counter_2 << "\n";*///if((ibloc==1)&&(counter_2 == 37))imprimir_v(natom,1,xoc);
                ...
                {//if(counter_2==14)std::cout << i << " ";if(i==37)std::cout << boost::format("%4d->%12.4f") % i % (xoc.v[i][0]*1E-10) << std::endl;
                    ...
                    {//if(j==37)std::cout << boost::format("%4d|>%12.4f") % j % (xoc.v[j][0]*1E-10) << std::endl;
                        if (intr.inter[i][j] == 1)
                        {
                            ...
                            double rij = std::sqrt(rmod2);
//if((ibloc==1)&&(counter_2 == 37)&&(i==1051))std::cout << boost::format("%4d %4d %40.35f %40.35f %40.35f %40.35f %40.35f %40.35f") %(i+1)%(j+1)%(xoc.v[i][0]*1e-11)%(xoc.v[i][1]*1e-11)%(xoc.v[i][2]*1e-11)%(xoc.v[j][0]*1e-11)%(xoc.v[j][1]*1e-11)%(xoc.v[j][2]*1e-11) << std::endl;
                            int k = 1 - OFFSET;
...
//if((ibloc==1)&&(counter_2 == 37)&&(i==1051)&&(j==1076))std::cout << boost::format("%4d %4d %45.35f")%k%k0%dpot << std::endl;
//if((ibloc==1)&&(counter_2 == 37)&&(i==1051)&&(j==1076))std::cout << boost::format("%40.35f %40.35f %40.35f %40.35f %40.35f %40.35f")%(xoc.v[i][0]*1e-11)%(xoc.v[i][1]*1e-11)%(xoc.v[i][2]*1e-11)%(xoc.v[j][0]*1e-11)%(xoc.v[j][1]*1e-11)%(xoc.v[j][2]*1e-11) << std::endl;
...
//if((ibloc==20)&&(counter_2 == 0))imprimir_v(natom,1,xoc);
...
//for (int i = 0; i < natom; ++i)std::cout << xoc.v[i][0] << " " << xoc.v[i][1] << " " << xoc.v[i][2] << std::endl;std::terminate();
            ...
            for (int j = 0; j < 3; ++j)
            {
                ...
                {//std::cout << boost::format("%14.6f %14.6f")%(xoc.xm[i]) % (xoc.v[i][j]*1E-10) << std::endl;
                    ekin = ekin + 0.5 * xoc.xm[i] * std::pow(xoc.v[i][j] * input.a, 2.0);
                }
            }//std::terminate();
//std::cout << "f" << std::endl;
