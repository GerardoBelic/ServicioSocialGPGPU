								 ...
								 unsigned numParticles, float mass, float deltaTime)
{
	...
	/// If there is a block that is incomplete, we handle the case appart
	unsigned uncompleted_threads = numParticles % workgroupSize;

    for (unsigned tile = 0; tile < numParticles - uncompleted_threads; tile += workgroupSize)
    {
    ...
    /// If there is an uncomplete work-group, we handle the special case
    if (uncompleted_threads != 0)
    {
        temp_tile[threadIdx.x] = positions[numParticles - uncompleted_threads + threadIdx.x];
		__syncthreads();

		for (unsigned j = 0; j < uncompleted_threads; ++j)
		{
			if (i == j)
				continue;
