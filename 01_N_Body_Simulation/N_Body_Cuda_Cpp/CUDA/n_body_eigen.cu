...
__device__ float dist2(Eigen::Vector3f A, Eigen::Vector3f B)
{
    Eigen::Vector3f C = A - B;
    return dot(C, C);
}

__global__ void n_body_vel_calc(Eigen::Vector3f *positions, Eigen::Vector3f *velocities,
								 unsigned numParticles, float mass, float deltaTime)
{
	...
	Eigen::Vector3f cur_position = positions[i];
	Eigen::Vector3f force = make_Eigen::Vector3f(0.0f, 0.0f, 0.0f);
    ...
    for (unsigned j = 0; j < numParticles; ++j)
    {
        ...
        Eigen::Vector3f direction = normalize(positions[j] - cur_position);
...
__global__ void n_body_pos_calc(Eigen::Vector3f *positions, Eigen::Vector3f *velocities,
...
int main(int argc, char **argv)
{
	...
	Eigen::Vector3f* d_positions = nullptr;
	Eigen::Vector3f* d_velocities = nullptr;

	unsigned size_memory = numParticles * sizeof(Eigen::Vector3f);
